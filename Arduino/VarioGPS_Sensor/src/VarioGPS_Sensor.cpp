...
/*
  ...
  ******************************************************************
  ...
  V2.1    23.12.17  Analog Messeingänge stark überarbeitet,
  NTC-Temperaturmessung hinzugefügt, startup-/auto-/maual-reset für
  Kapazitätsanzeige, SRAM-Speicheroptimierung V2.0.2  03.12.17  Fehler in GPS
  Trip behoben V2.0.1  21.11.17  Fehler bei Spannungsmessung behoben V2.0
  20.11.17  GPS Trip[km] und verbrauchte Kapazität[mAh] eingebaut, Stromsensoren
  ACS712 eingebaut V1.9    17.11.17  ACSxxx Stromsensoren eingebaut V1.8
  17.11.17  Luftdrucksensor MS5611/LPS werden unterstützt V1.7.2  15.11.17
  Fehlerbehebung mit BME280 V1.7.1  15.11.17  Speicheroptimierung, kleinere
  Fehler behoben V1.7    12.11.17  Empfängerspannungen können gemessen werden
  V1.6    05.11.17  Luftdrucksensoren BMP085/BMP180/BMP280/BME280 eingebaut, und
  zu VarioGPS umbenannt V1.5    05.11.17  Code von RC-Thoughts(Jeti_GPS-Sensor)
  übernommen
  ...
  ******************************************************************
  ...
  - Stromsensoren @3.3V/5V Betriebsspannung:        AttoPilot Module @3.3V:
  45A/13.6V - 90A/50V - 180A/50V (@5V: 45A/20.6V - 90A/60V - 180A/60V) APM2.5
  PowerModul @5V: 90A/50V (@3.3V: 58A/33.4V) ACS758_50B, ACS758_100B,
  ACS758_150B, ACS758_200B, ACS758_50U, ACS758_100U, ACS758_150U, ACS758_200U
  - zusätzliche Stromsensoren @5V Betriebsspannung: ACS712_05, ACS712_20,
  ACS712_30
  ...
  ******************************************************************
  ...
  - Strom- und Spannung für Hauptantrieb mit verbrauchter Kapazität[mAh] und
  Leistung[W]
  ...
  - Einstellung Reset der Kapazität:
        ...
        AUTO(Wert wird gespeichert und erst zurückgesetzt wenn ein geladener
  Akku angeschlossen wird) MANUAL(Wert muss manuell per Jetibox zurückgesetzt
  werden mit RESET OFFSET)
...
const float factorVoltageDivider[] = {
    float(voltageInputR1[0] + voltageInputR2[0]) / voltageInputR2[0],
...
const float timefactorCapacityConsumption =
    (1000.0 / MEASURING_INTERVAL * 60 * 60) / 1000;
...
// identify sensor
  ...
  if (pressureSensor.type == unknown) {
    if (lps.init()) {
      ...
      Wire.beginTransmission(MS5611_ADDRESS); // if no Bosch sensor, check if
                                              // return an ACK on MS5611 address
...
// read settings from eeprom
  ...
  if (currentSensor == mainDrive_disabled) {
    ...
    if (capacityMode > startup) {
      ...
      if (capacityMode == automatic) {
        ...
        float cuVolt =
            readAnalog_mV(getVoltageSensorTyp(), VOLTAGE_PIN) / 1000.0;
        float oldVolt;
  ...
  jetiEx.Start("VarioGPS", sensors, JetiExProtocol::/*SERIAL2*/ DEFAULTPORT);
}
...
void loop() {
  ...
  if ((millis() - lastTime) > MEASURING_INTERVAL) {
    ...
    if (pressureSensor.type != unknown) {
      ...
      case MS5611_:
        uPressure =
            ms5611.readPressure(true); // In Pascal (100 Pa = 1 hPa = 1 mbar)
        curAltitude =
            ms5611.getAltitude(uPressure, 101325) * 100; // In Centimeter
        uTemperature = ms5611.readTemperature(true) *
                       10; // In Celsius ( x10 for one decimal)
        break;
      case LPS_:
        uPressure =
            lps.readPressureMillibars(); // In Pascal (100 Pa = 1 hPa = 1 mbar)
        curAltitude =
            lps.pressureToAltitudeMeters(uPressure) * 100; // In Centimeter
        uTemperature =
            lps.readTemperatureC() * 10; // In Celsius ( x10 for one decimal)
        break;
#endif
#ifdef SUPPORT_BMx280
      default:
        uPressure = boschPressureSensor
                        .readPressure(); // In Pascal (100 Pa = 1 hPa = 1 mbar)
        curAltitude = boschPressureSensor.readAltitude() * 100; // In Centimeter
        uTemperature = boschPressureSensor.readTemperature() *
                       10; // In Celsius ( x10 for one decimal)
        if (pressureSensor.type == BME280) {
          jetiEx.SetSensorValue(
              ID_HUMIDITY, boschPressureSensor.readHumidity() * 10); // In %rH
        }
        break;
#endif
      }
      ...
      uVario =
          (curAltitude - lastAltitude) * (1000 / float(millis() - lastTime));
      lastAltitude = curAltitude;
    ...
    if (currentSensor) {
      ...
      if (capacityMode > startup &&
          (millis() - lastTimeCapacity) > CAPACITY_SAVE_INTERVAL) {
    ...
    if (enableRx1) {
      jetiEx.SetSensorValue(ID_RX1_VOLTAGE,
                            readAnalog_mV(Rx1_voltage, RX1_VOLTAGE_PIN) / 10);
    }

    if (enableRx2) {
      jetiEx.SetSensorValue(ID_RX2_VOLTAGE,
                            readAnalog_mV(Rx2_voltage, RX2_VOLTAGE_PIN) / 10);
    }
    ...
    if (enableExtTemp) {
      ...
      steinhart -=
          273.15 - SELF_HEAT; // convert to °C and self-heat compensation
  ...
  if (gpsSettings.mode != GPS_disabled) {
    ...
    if (gps.location.isValid() && gps.location.age() < 2000) { // if Fix
      ...
      if (homeSetCount < 3000) { // set home position
        ...
        // Distance to model
        distToHome = gps.distanceBetween(gps.location.lat(), gps.location.lng(),
                                         home_lat, home_lon);
        ...
        jetiEx.SetSensorValue(ID_COURSE, gps.courseTo(home_lat, home_lon,
                                                      gps.location.lat(),
                                                      gps.location.lng()));
        ...
        if (gpsSettings.distance3D) {
          distLast =
              sqrt(pow(uAbsAltitude - lastAbsAltitude, 2) + pow(distLast, 2));
